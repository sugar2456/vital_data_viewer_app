...
**独立テスト**: 睡眠可視化画面を開き、今日の睡眠データが縦軸に睡眠ステージ、横軸に時刻を持つグラフ形式で表示されることを確認することで、完全にテストできます。

**受入シナリオ**:

1. **前提条件**: ユーザーのFitbitアカウントに今日の睡眠データがある  
   **操作**: ユーザーが睡眠可視化画面を開く  
   **期待結果**: グラフに今日の日付の睡眠ステージが縦軸に、時刻が横軸に表示される（stages形式またはclassic形式のいずれか）  
...
**独立テスト**: 右下のカレンダーアイコンをタップし、過去の日付を選択し、グラフがその日付の睡眠データを表示するように更新されることを確認することで、完全にテストできます。

**受入シナリオ**:

1. **前提条件**: ユーザーが睡眠可視化画面を表示している  
   **操作**: ユーザーが右下のカレンダーアイコンをタップする  
   **期待結果**: 日付ピッカーが表示される  
...
**独立テスト**: 睡眠グラフを表示し、各睡眠ステージ(深い睡眠、浅い睡眠、REM睡眠、覚醒)が明確にラベル付けされ、色またはパターンによって視覚的に区別できることを確認することで、完全にテストできます。

**受入シナリオ**:

1. **前提条件**: 睡眠データが表示されている
   **操作**: ユーザーがグラフを表示する
   **期待結果**: 各睡眠ステージ（stages形式: deep/light/rem/wake、またはclassic形式: asleep/restless/awake）が明確にラベル付けされ、明確な視覚的表現を使用している  

2. **前提条件**: グラフが睡眠データを表示している  
   **操作**: ユーザーが時間軸を表示する  
   **期待結果**: 各睡眠ステージがいつ発生したかを示すために、適切な間隔(例: 1時間ごと)で時間ラベルが表示される  

3. **前提条件**: ユーザーが睡眠ステージを表示している
   **操作**: グラフを見ている
   **期待結果**: 睡眠ステージ間の遷移が明確に見える
...
**独立テスト**: 睡眠可視化画面を開き、画面上にトータル睡眠時間（例: "7時間30分"）が表示されることを確認することで、完全にテストできます。

**受入シナリオ**:

1. **前提条件**: ユーザーが睡眠データのある日付を表示している  
   **操作**: 画面を確認する  
   **期待結果**: トータル睡眠時間が時間と分の形式（例: "7時間30分"）で明確に表示される  
...
- **FR-003**: システムはグラフの縦軸に睡眠ステージを表示しなければならない（データ形式に応じて適切なステージを表示）
- **FR-004**: システムはグラフの横軸に時刻を表示しなければならない
- **FR-005**: システムは画面の右下にカレンダーアイコンを提供しなければならない
...
- **FR-010**: システムは各睡眠ステージを明確な視覚的表現(色またはパターン)で明確にラベル付けしなければならない
- **FR-011**: システムは横軸に適切な間隔で時間ラベルを表示しなければならない
- **FR-012**: システムは選択した日付の睡眠データがない場合にメッセージを表示しなければならない
- **FR-013**: システムは選択した日付のトータル睡眠時間を表示しなければならない
...
- **SC-003**: 各睡眠ステージ（深い睡眠、浅い睡眠、REM睡眠、覚醒）が異なる色またはパターンで視覚的に区別できる
- **SC-004**: 選択した日付のトータル睡眠時間が画面上に明確に表示される
